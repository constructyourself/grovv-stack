...
- All three loop layers are now specified. Implemented: the detect branch only. Awaiting review — `docs/architecture/verify-loop.md` (Layer 1), `re-entry.md` (Layer 2), `pipeline-smoke-test.md` (Layer 3). Smallest standalone pieces first: verify-loop Phase 1 (discovery, asks nothing) and re-entry Phase 1 (three sentences naming the ten baseline skills, which `team-design.md:143` has always deduped against nothing).
- Unresolved and load-bearing before Layer 2 is built: `docs/tech-spec.md` is itself generated, so it cannot settle a drifted framework assertion without a generated file answering an ask-first question. `re-entry.md` proposes reporting the disagreement and naming both files rather than picking a winner.
- The Step 2 unknowns insertion (renumber 2–9 → 3–10) is still unexecuted, and the map has now been **re-derived against the current tree**. Corrected figures: **118 live pipeline references across 17 files**, not the ~60 the map and `loop-engineering.md:22` both assume. Separately: 261 step tokens sit in `docs/architecture/` analysis notes, and 20 in the vendored harness — the harness ones are its own Korean-language phases and a naive `sed` would corrupt Apache-2.0 vendored code.
- Both of the map's traps confirmed present. Exactly one ASCII-hyphen range exists, at `grovv-stack-scaffold.md:622`; its lower bound must stay 2 while its upper bound moves, because the preceding list item covers Step 1 and the bound means "everything after Step 1" rather than "old Step 2". A mechanical +1 on both ends is wrong there and only there. The other twelve ranges use en dashes and take the +1 on the upper bound only. A `sed` written for one dash form silently misses the other.
- Undecided and worth settling before executing: whether `docs/architecture/`'s 261 references get renumbered. They are historical analysis citing the pipeline as it stood; renumbering falsifies the record, leaving them makes them wrong. Recommendation: leave them, and add a dated note at the top of each affected doc.
