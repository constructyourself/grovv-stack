...
- **Finish the smoke test.** Run 1 covered Steps 0–2 only; Steps 3–9, run 2 (re-entry) and run 3 (adopt) are unexercised, and that is where Step 6's CI question, the fourteen skills, the re-entry contract and every G/S/T/M/W/R row live. The partial run already paid for itself five times over. Target and record are gone with the session — re-run from the rubric.
