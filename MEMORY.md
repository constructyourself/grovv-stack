...
- Repo is a multi-tool prompt-driven scaffolding system (`grovv-stack` v0.9.0) — not an application. Output is docs/config in *other* projects.
...
- **The smoke test is now the only open item of substance.** Layers 1 and 2 of `loop-engineering.md` are both complete, and every remaining risk is a generation regression that only a real run can catch. Run `docs/architecture/pipeline-smoke-test.md` against a throwaway target, including the second-run case with the spec edited in between, which nothing has ever exercised. The rubric predates Step 2's pass, Step 6's CI question and the re-entry contract, so it needs new rows before it is scored.
