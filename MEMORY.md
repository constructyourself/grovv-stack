...
- Repo is a multi-tool prompt-driven scaffolding system (`grovv-stack` v0.8.0) — not an application. Output is docs/config in *other* projects.
...
- Build the two `## Re-entry` sections in `skills-builder.md` and `team-design.md` (`re-entry.md` Phases 2–4). Everything they depended on is in place: the detect branch, the named baseline skills, and the ask-first clash rule.
- Still proposed from the unknowns workstream, both standalone and neither needing a renumber: implementation notes into `agents/testing.md` and the change-comprehension quiz into `agents/code-review.md`, four trees each (`unknowns-engineering.md` Phase 1); and the four new baseline skills specified in `unknowns-skill-bodies.md` (Phase 3), which would take the baseline set from 10 to 14.
- **If a renumber is ever proposed again, count in lines and say so.** Two figures in this file contradicted each other because they measured different units, and both were right: at `HEAD` on 2026-07-27 the pipeline surface is **123 occurrences across 100 lines in 18 files** (excluding `.git`, the vendored harness, and `docs/architecture/`), and `docs/architecture/` holds **261 occurrences across 158 lines in 5 files**. The harness trees hold 20 more, which are that skill's own Korean-language phase numbers in Apache-2.0 code — a naive `sed` corrupts vendored source.
- The map's two traps are confirmed and still live. There is exactly one ASCII-hyphen range, `grovv-stack-scaffold.md:622` ("Work through Steps 2-9"); its lower bound must stay 2 while the upper bound moves, because the item before it covers Step 1, so the bound means "everything after Step 1". The other 13 ranges are `Steps 0–9` with en dashes. A `sed` for one dash form silently misses the other, and matching en dashes inside a POSIX bracket expression is locale-dependent — count with `grep -o` and eyeball the separators.
