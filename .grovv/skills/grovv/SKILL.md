...
```
${GROVV_PLUGIN_ROOT}/grovv-stack-scaffold.md
```

When running from a clone of the grovv-stack repo rather than an installed plugin, read `grovv-stack-scaffold.md` at the repo root. It defines the full workflow (Steps 0–9) and is authoritative. Also follow the conventions in the tool-specific context file (`CLAUDE.md`, `VIBE.md`, or `CODEX.md`).
...
This canonical skill works across all supported AI coding assistants:

- **Vibe**: Uses `.vibe/skills/` and `.vibe/agents/`
- **Codex**: Uses `.codex/skills/` and `.codex/agents/`
- **Claude Code**: Uses `.claude/skills/` and `.claude/agents/`

The tool is auto-detected at runtime. For tool-specific customizations, see the respective tool directories.
