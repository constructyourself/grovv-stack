...
**Status, 2026-07-27.** Layer 1 is **complete** — Step 1 discovers the target's verify commands, Step 8 records them in `MEMORY.md`, Step 6 asks a four-option CI question and generates a workflow from exactly those commands, and Step 9's README reads the table instead of inventing `npm` scripts. `docs/architecture/verify-loop.md` is the specification and carries the detail. Layer 2 is partial: the third detect branch and the named baseline-skill subject both landed, and the ask-first clash is settled and implemented, but the two `## Re-entry` sections are not written. Layer 3 is partial: seven check-only scripts run in CI; the smoke-test rubric exists at `docs/architecture/pipeline-smoke-test.md` and has still never been run.
