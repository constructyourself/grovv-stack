...
| S17 | **Every skill folder contains a `SKILL.md`** | Count folders under the skills directory, count `SKILL.md` files, compare. A folder holding only `references/` is a skill that will never load or trigger, and nothing downstream reads these files, so the failure is silent. Run 1 produced exactly this: `change-quiz` generated a good reference file and no skill | Blocker |
