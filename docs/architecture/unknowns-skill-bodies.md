...
**Status, 2026-07-27: applied.** The baseline set is fourteen. Both enumerations say so — the table and tree in `docs/prompts/skills-builder.md`, and the mirrored table, project tree and expected-output count in `grovv-stack-scaffold.md` — and `docs/prompts/team-design.md` names all fourteen at both of its dedupe sites plus its checklist. The register update landed: `architecture-planning` now appears at both ask-first sites and in the matching checklist item. The footer exemption names the four new bodies. Part 3's checklist items are in the deliverable checklist.

One deliberate departure from Part 2. The four fold-in sections are carried in `skills-builder.md` as a **required-content table plus the prototype boundary stated in full**, not as the ~130 lines of verbatim body text this document specifies. The reason is the house idiom: `skills-builder.md` states what a generated skill must contain and points at the canonical statement elsewhere — exactly how it already handles the Throwaway Tier — rather than carrying finished bodies. The hard boundary that has been read wrong before, that a prototype is not a framework commitment, is stated in full rather than summarised. The bodies below remain the reference for an implementer writing the actual sections.
