...
The pipeline runs: structure + config → product spec → development plan → tech spec → prompt docs → skills-builder → **team-design (harness)** → readme. The team-design step uses the bundled [harness](.claude/skills/harness/ATTRIBUTION.md) meta-skill to design a project-specific agent team, additive to the six grovv defaults.
